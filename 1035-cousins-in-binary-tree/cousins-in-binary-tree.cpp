...
public:
 TreeNode* find_parent(TreeNode* cur,int val) {
     if (!cur or cur->val==val)return 0;
     if (cur->left) 
         if (cur->left->val == val)return cur;
     
     if (cur->right) 
         if (cur->right->val == val)return cur;
  ...
  int depth(TreeNode* c, int value,int cur=0) {
    ...
    bool isCousins(TreeNode* root, int x, int y) {
        if(depth(root,x)!=depth(root,y))
            return false;
        if(find_parent(root,x)==find_parent(root,y))return false;
        return true;
    }
};